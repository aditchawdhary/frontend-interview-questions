...
export default async function mapAsyncLimit(iterable, callbackFn, size) {
    ...
    let iterLength = iterable.length;
    let results = new Array();
    if (iterLength === 0) {
       return results;
    }

    let totalTokens = size || iterLength;

    for(let i=0; i<iterLength; i+=totalTokens) {
          const chunk = iterable.slice(i, i+totalTokens);
          const chunkResult = await Promise.all(chunk.map(callbackFn));
