...
Promise.any() takes an iterable of elements (usually Promises). It returns a single promise that resolves as soon as any of the elements in the iterable fulfills, with the value of the fulfilled promise. 
...
Let's implement our own version of Promise.any(), a promiseAny function, with the difference being the function takes in an array instead of an iterable and AggregateErrors returned just have to return an 
array of error reasons, the message doesn't have to be set. Refer to the AggregateError constructor examples on MDN.
...
export default function promiseAny(iterable) {
  return new Promise((resolve, reject) => {
    ...
    let pending = length, allErrors = new Array(length);
    async function callback(val, index) {
      try {
        let cur_val = await val;
        resolve(cur_val);
      } catch(error) {
        pending--;
